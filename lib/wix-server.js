...
export async function serviceFetch(url, options, service, fetcher = fetch) {
  try {
    return await fetcher(url, { ...options, signal: AbortSignal.timeout(12000), redirect:'error' });
  } catch (error) {
    if (['TimeoutError','AbortError'].includes(error?.name)) throw new WixError(504,`${service}: Zeitüberschreitung beim Abruf. Bitte erneut versuchen.`);
    throw new WixError(503,`${service}: Verbindung konnte nicht hergestellt werden. Bitte erneut versuchen.`);
  }
}
export async function requestJson(url, options, service, fetcher = fetch) {
  const response = await serviceFetch(url,options,service,fetcher);
  if (!response.ok) {
    const error = new WixError(502, `${service} antwortet mit HTTP ${response.status}. Zugang und Berechtigungen prüfen.`);
  ...
  try { return await response.json(); }
    ...
    if (['TimeoutError','AbortError'].includes(error?.name)) throw new WixError(504,`${service}: Zeitüberschreitung beim Lesen der Antwort. Bitte erneut versuchen.`);
    throw new WixError(502,`${service}: Antwort konnte nicht als JSON gelesen werden. Bitte erneut versuchen.`);
  }
}
...
export async function requireAdmin(req, env, fetcher = fetch) {
  ...
  const response = await serviceFetch(`${env.SUPABASE_URL.replace(/\/$/,'')}/auth/v1/user`, {
    headers:{apikey:env.SUPABASE_ANON_KEY,Authorization:token},
  },'Supabase-Anmeldeprüfung',fetcher);
  if (!response.ok) {
    if ([401,403].includes(response.status)) throw new WixError(401,'Bitte erneut anmelden.');
    throw new WixError(503,`Supabase-Anmeldeprüfung antwortet mit HTTP ${response.status}. Bitte erneut versuchen.`);
  }
  let user;
  try { user = await response.json(); }
  catch { throw new WixError(502,'Supabase-Anmeldeprüfung: Antwort konnte nicht gelesen werden. Bitte erneut versuchen.'); }
...
export async function wixRequest(env, path, body, fetcher = fetch, service = 'Wix') {
...
export async function resolveWixPlanOrder(env, id, fetcher = fetch) {
  try {
    const result = await wixRequest(env,`/pricing-plans/v2/orders/${id}`,undefined,fetcher,'Wix-Preisplanbestellung');
    if (result.order?.id !== id) throw new WixError(404,'Wix hat keine passende Preisplanbestellung zurückgegeben.');
    return result.order;
  } catch (error) {
    if (error.upstreamStatus !== 404) throw error;
  }
  // A dashboard subscriptions URL can identify the subscription instead of the order.
  // Bound the lookup to keep webhook executions within the hosting time limit.
  for (let offset=0; offset<150; offset+=50) {
    const result = await wixRequest(env,`/pricing-plans/v2/orders?limit=50&offset=${offset}`,undefined,fetcher,'Wix-Preisplanliste');
    if (!Array.isArray(result.orders)) throw new WixError(502,'Wix hat keine gültige Preisplanliste zurückgegeben.');
    const matches = result.orders.filter(order=>order.subscriptionId===id);
    if (matches.length>1) throw new WixError(409,'Mehrere Bestellungen für dieses Abonnement gefunden. Bitte die Bestell-ID verwenden.');
    if (matches.length===1) {
      const orderId=matches[0].id;
      if (!validId(orderId)) throw new WixError(502,'Wix-Preisplanbestellung enthält keine gültige ID.');
      const {order}=await wixRequest(env,`/pricing-plans/v2/orders/${orderId}`,undefined,fetcher,'Wix-Preisplanbestellung');
      if (order?.id!==orderId || order?.subscriptionId!==id) throw new WixError(409,'Wix-Abonnement und Bestellung stimmen nicht überein.');
      return order;
    }
    if(result.orders.length<50) throw new WixError(404,'Keine Preisplanbestellung mit dieser Bestell- oder Abonnement-ID gefunden.');
  }
  throw new WixError(404,'Abonnement nicht unter den letzten 150 Preisplanbestellungen gefunden. Bitte die tatsächliche Bestell-ID verwenden.');
}
export async function resolveWixBuyer(env, buyer, fetcher = fetch) {
  if (validId(buyer?.contactId)) {
    try {
      const {contact}=await wixRequest(env,`/contacts/v4/contacts/${buyer.contactId}`,undefined,fetcher,'Wix-Kontaktabruf');
      if (!contact) throw new WixError(502,'Wix-Kontaktabruf hat keine Kontaktdaten zurückgegeben.');
      return contact;
    } catch (error) {
      if (error.upstreamStatus!==404 || !validId(buyer?.memberId)) throw error;
    }
  }
  if (!validId(buyer?.memberId)) return null;
  // Use only the member explicitly linked by the authoritative Wix order.
  // A nickname or a search by name/email cannot establish the buyer's identity.
  const {member}=await wixRequest(env,`/members/v1/members/${buyer.memberId}?fieldsets=FULL`,undefined,fetcher,'Wix-Mitgliederabruf');
  if (member?.id!==buyer.memberId) throw new WixError(422,'Wix hat kein eindeutig zur Bestellung gehörendes Mitglied zurückgegeben.');
  return {
    info:{name:{first:member.contact?.firstName,last:member.contact?.lastName}},
    primaryInfo:{email:member.loginEmail},
  };
}
export function isFreeWixOrder(entity) {
  const zero = value => value === 0 || (typeof value === 'string' && /^0+(?:\.0+)?$/.test(value));
  // Every listed price period must be free; a free first period alone is insufficient.
  if (entity.pricing?.prices !== undefined) {
    const prices=entity.pricing.prices;
    if (!Array.isArray(prices) || !prices.length || !prices.every(item=>zero(item?.price?.total))) return false;
    return entity.priceDetails?.total === undefined || zero(entity.priceDetails.total);
  }
  return zero(entity.priceDetails?.total);
}
export function isPendingFreeSinglePayment(entity) {
  const pricing=entity.pricing;
  const duration=pricing?.singlePaymentForDuration;
  const fixedDuration=duration && Number.isInteger(duration.count) && duration.count>0 && ['DAY','WEEK','MONTH','YEAR'].includes(duration.unit);
  // A pending payment is never proof of payment. This exception establishes that
  // there is no payment due for a fully free, non-recurring online purchase.
  return entity.type==='ONLINE' && entity.lastPaymentStatus==='PENDING'
    && ['PENDING','ACTIVE'].includes(entity.status)
    && Boolean(fixedDuration || pricing?.singlePaymentUnlimited===true)
    && !pricing?.subscription && !entity.freeTrialDays && !pricing?.freeTrialDays
    && Array.isArray(pricing?.prices) && pricing.prices.length===1
    && isFreeWixOrder(entity);
}
...
export function normalizeWix(kind, entity, contact = null) {
  ...
  if (kind === 'plan') {
    ...
    const paymentConfirmed = entity.lastPaymentStatus === 'PAID'
      || (entity.lastPaymentStatus === 'NOT_APPLICABLE' && isFreeWixOrder(entity))
      || isPendingFreeSinglePayment(entity);
    if (['PENDING','ACTIVE'].includes(entity.status) && paymentConfirmed) base.action='upsert';
    else if (entity.status === 'CANCELED') base.action='cancel';
    else if (entity.status === 'DRAFT') base.action='ignore';
    else base.reason=`Preisplan braucht Prüfung. Wix-Status: ${trim(entity.status,40)||'fehlt'}; Zahlungsstatus: ${trim(entity.lastPaymentStatus,40)||'fehlt'}. Vollständig kostenlos laut Preisdaten: ${isFreeWixOrder(entity)?'ja':'nicht bestätigt'}. Keine eindeutig bezahlte oder zulässige kostenlose aktive/geplante Bestellung.`;
  } else {
...
export async function importWix(env, kind, id, fetcher = fetch) {
  ...
  let entity, contact;
  if (kind === 'plan') {
    entity = await resolveWixPlanOrder(env,id,fetcher);
    contact = await resolveWixBuyer(env,entity.buyer,fetcher);
  } else {
  ...
  if (kind === 'booking' && entity?.id !== id) throw new WixError(404,'Die Buchung wurde auf der konfigurierten Wix-Webseite nicht gefunden.');
...
// Preserve the amount actually paid in Wix, including coupon discounts.
const normalizeWixWithPaidAmount = normalizeWix;
normalizeWix = function(kind, entity, contact = null) {
  const normalized = normalizeWixWithPaidAmount(kind, entity, contact);
  if (kind === 'plan') {
    const paidAmount = Number(entity?.priceDetails?.total);
    if (Number.isFinite(paidAmount) && paidAmount >= 0) normalized.paid_amount = Math.round(paidAmount * 100) / 100;
  }
  return normalized;
};
