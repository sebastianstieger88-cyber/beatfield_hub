...
export function normalizeWix(kind, entity, contact = null) {
  ...
  if (kind === 'plan') {
    ...
    const paymentConfirmed = entity.lastPaymentStatus === 'PAID' || (entity.lastPaymentStatus === 'NOT_APPLICABLE' && isFreeWixOrder(entity));
    ...
    else base.reason=`Preisplan braucht Prüfung. Wix-Status: ${trim(entity.status,40)||'fehlt'}; Zahlungsstatus: ${trim(entity.lastPaymentStatus,40)||'fehlt'}. Keine eindeutig bezahlte oder kostenlose aktive/geplante Bestellung.`;
  } else {
