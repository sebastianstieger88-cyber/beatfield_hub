...
async function handleSeasonBookingCreate(event) {
  ...
  try {
    ...
    const selectedDays = Array.from(seasonBookingForm.querySelectorAll('input[name="selectedDays"]'))
    ...
    if (bookingId) {
      ...
      if (bookingUpdateResult.error) {
        notify(getFriendlySupabaseMessage(bookingUpdateResult.error, "Buchung konnte nicht gespeichert werden."), true);
        return;
      }

      optimisticBooking = {
        ...(state.seasonBookings.find((entry) => entry.id === bookingId) || {}),
        id: bookingId,
        season_id: seasonId,
        full_name: fullName,
        phone: phone || null,
...
function renderStats() {
  ...
  state.courses.forEach((course) => {
    ...
    const aggregate = getCourseAttendanceAggregate(course.id);
    const average = aggregate.rate ?? 0;
...
function renderBusinessDashboard() {
  ...
  const monthAggregate = getMonthAttendanceAggregate(selectedMonth);
  const avgAttendance = monthAggregate.rate ?? 0;
  ...
  const summaryCards = [
    ...
    { title: "Termine im Monat", value: monthSessions.length, meta: `Monat | alle Termine im ${getSelectedMonthLabel()}` },
    { title: "Fehlquote", value: `${noShowRate}%`, meta: `Monat | dazu ${avgAttendance}% dokumentierte Anwesenheit` },
...
function renderMonthlyOverview() {
  ...
  const items = [
    ...
    { title: "Monatsquote", value: averageDisplay, meta: totalMarked ? "Monat | dokumentierte Anwesenheit im Monat" : "Monat | noch keine dokumentierten Anwesenheiten" },
    { title: "Bester Teilnehmer", value: getTopParticipantName({ monthValue: getSelectedMonth() }), meta: `Monat | beste Monatsquote im ${getSelectedMonthLabel()}` },
  ];
  ...
  const items = [
    {
      ...
      meta: `Report | Grundlage ist ${getSelectedMonthLabel()}`,
    },
    {
      title: "Aktiver Filter",
      value: state.participantSearch || "Kein Filter",
      meta: "Report | wirkt auf Listen und Export",
    },
    {
      title: "Unter 60%",
      value: getLowAttendanceParticipants({ monthValue: getSelectedMonth() }).length,
      meta: `Report | Teilnehmer unter 60% im ${getSelectedMonthLabel()}`,
    },
  ];
...
function exportSelectedCourseCsv() {
  ...
  participants.forEach((participant) => {
    ...
    rows.push([
      ...
      participant.phone,
      ...marks,
      `${getParticipantAnalysisSummary(participant).rate}%`,
    ]);
  });
...
function exportLeaderboardCsv() {
  const rows = [["Kurs", "Teilnehmer", "Telefon", "Anwesenheitsquote", "Gezählte Termine"]];

  getAnalysisParticipantEntries()
...
function exportTrainerReportCsv() {
  ...
  trainerSummaries.forEach((trainer) => {
    ...
    const trainerAggregate = trainerCourses.reduce((aggregate, course) => {
      const courseAggregate = getCourseAttendanceAggregate(course.id);
      return {
        present: aggregate.present + courseAggregate.present,
        total: aggregate.total + courseAggregate.total,
      };
    }, { present: 0, total: 0 });
    const average = trainerAggregate.total
      ? Math.round((trainerAggregate.present / trainerAggregate.total) * 100)
      : 0;
...
function calculateAttendanceRate(courseId, participantId) {
  ...
  return getParticipantAnalysisSummary(participant).rate;
}
...
function getSeasonBookingAttendanceSummary(booking, participant) {
  ...
  const linkedParticipantIds = new Set(getLinkedParticipantsForBooking(booking, participant).map((entry) => entry.id));
  if (participant?.id) {
...
function getLinkedParticipantsForBooking(booking, participant = null) {
  if (!booking?.id) {
    return participant ? [participant] : [];
  }

  const linkedParticipants = state.participants.filter((entry) => entry.season_booking_id === booking.id);
  if (participant?.id && !linkedParticipants.some((entry) => entry.id === participant.id)) {
    linkedParticipants.push(participant);
  }
...
function getAnalysisParticipantEntries(options = {}) {
  ...
  state.courses.forEach((course) => {
    getParticipantsForCourse(course.id).forEach((participant) => {
      const booking = getParticipantSeasonBooking(participant);
      const key = booking?.id ? `booking:${booking.id}` : `participant:${participant.id}`;
      if (entries.has(key)) {
        return;
      }

      const linkedParticipants = booking ? getLinkedParticipantsForBooking(booking, participant) : [participant];
      ...
      entries.set(key, {
        ...
        courseLabel: courseNames.join(" / ") || (state.courses.find((entry) => entry.id === participant.course_id)?.name || "-"),
        summary,
        rate: summary.rate,
      });
    });
...
function getMonthAttendanceAggregate(monthValue) {
  return state.courses.reduce((aggregate, course) => {
    const courseAggregate = getCourseAttendanceAggregate(course.id, { monthValue });
    return {
      present: aggregate.present + courseAggregate.present,
      total: aggregate.total + courseAggregate.total,
      rate: null,
...
function groupSeasonBookingsByContact(bookings) {
  ...
  bookings.forEach((booking) => {
    const key = `${String(booking.full_name || "").trim().toLowerCase()}::${String(booking.phone || "").trim()}`;
    if (!groups.has(key)) {
      groups.set(key, {
        key,
        fullName: booking.full_name || "Ohne Namen",
        phone: booking.phone || "",
        bookings: [],
      });
    }
  ...
  return Array.from(groups.values())
    .map((group) => ({
      ...
      bookings: group.bookings.sort((left, right) => {
        const leftSeason = state.seasons.find((entry) => entry.id === left.season_id);
        const rightSeason = state.seasons.find((entry) => entry.id === right.season_id);
        const leftDate = leftSeason?.start_date || left.start_date || left.created_at || "";
        const rightDate = rightSeason?.start_date || right.start_date || right.created_at || "";
